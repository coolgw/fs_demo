...
/*
 * Demofs (Version 2) - Triggers Post-Remount Size 0 (Transient Inode Bug)
...
static const struct address_space_operations demofs_aops = {
	...
	.write_end	= generic_write_end, /* CORRECT write_end: No more unmount hang */
...
/* BUGGY drop_inode: Forces VFS to evict inodes immediately, discarding dirty states and size updates! */
...
{
	pr_info("demofs_v2: registering block-backed filesystem\n");
	return register_filesystem(&demofs_fs_type);
}

static void __exit demofs_exit(void)
{
	pr_info("demofs_v2: unregistering block-backed filesystem\n");
...
MODULE_DESCRIPTION("A physical block-device backed virtual filesystem (Version 2)");
