...
/*
 * Demofs (Version 3) - Triggers Data Loss (Missing Writepages Callback)
...
static const struct super_operations demofs_ops = {
	...
	/* CORRECT super_ops: No drop_inode callback! Inode size is written correctly. */
...
{
	pr_info("demofs_v3: registering block-backed filesystem\n");
	return register_filesystem(&demofs_fs_type);
}

static void __exit demofs_exit(void)
{
	pr_info("demofs_v3: unregistering block-backed filesystem\n");
...
MODULE_DESCRIPTION("A physical block-device backed virtual filesystem (Version 3)");
