...
/*
 * Demofs (Version 1) - Triggers Unmount Hang (Folio Lock Leak)
...
/* BUGGY WRITE_END: Folio unlock/put commented out to deliberately trigger the unmount hang! */
			    ...
			    struct folio *folio, void *fsdata)
{
	...
	/* Explicitly update in-memory and disk-based file sizes */
	...
	/* --- DELIBERATE BUG (Commented out fixes) --- */
	// folio_unlock(folio);
	// folio_put(folio);
	/* ──────────────────────────────────────────── */
	return ret;
...
static const struct address_space_operations demofs_aops = {
	...
	.write_end	= demofs_write_end, /* Commented out unlock/put version */
...
{
	pr_info("demofs_v1: registering block-backed filesystem\n");
	return register_filesystem(&demofs_fs_type);
}

static void __exit demofs_exit(void)
{
	pr_info("demofs_v1: unregistering block-backed filesystem\n");
...
MODULE_DESCRIPTION("A physical block-device backed virtual filesystem (Buggy Version 1)");
